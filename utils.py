...
def clean2(text):
    tokens = [word for word in nltk.word_tokenize(
        text) if word.lower() not in stopwords]
...
def splitXY(X, Y, split):
    assert len(X) == len(
        Y), 'ValidationError: lengths of X and Y are not equal'
...
domain = None
sklearn_tfidf_ada = pickle.load(open(os.path.join(
    os.path.dirname(__file__), 'models/tfidf_filterer_ada.pkl'), 'rb'))
sklearn_tfidf_os = pickle.load(open(os.path.join(
    os.path.dirname(__file__), 'models/tfidf_filterer_os.pkl'), 'rb'))
...
def get_filtered_questions(questions, threshold=0.25, what_type='os'):
    ...
    for i in range(0, len(questions)):
        ...
        word_dict.update(
            {w: s for w, s in [(feature_names[j], s) for (j, s) in tfidf_scores]})
...
def get_data_for_cognitive_classifiers(threshold=[0, 0.1, 0.15], what_type=['ada', 'os', 'bcl'], what_for='train', include_keywords=True, keep_dup=False, shuffle=True):
            ...
            for row in csvreader:
                if row:
                    ...
                    clean_sentence = clean(
                        sentence, stem=False, lemmatize=False, return_as_list=False)
                    X_data_temp.append(clean_sentence)
                    Y_data_temp.append(mapping_cog[label_cog])

        for t in threshold:
            X_data_temp_2 = get_filtered_questions(
                X_data_temp, threshold=t, what_type='ada')
    ...
    if 'os' in what_type:
        with open(os.path.join(os.path.dirname(__file__), 'datasets/OS_Exercise_Questions_Labelled%s.csv' % suffix), 'r', encoding='latin-1') as csvfile:
            ...
            for row in csvreader:
                if row:
                    shrey_cog, shiva_cog, mohit_cog = row[2].split(
                        '/')[0], row[4].split('/')[0], row[6].split('/')[0]
                    label_cog = mohit_cog if mohit_cog else (
                        shiva_cog if shiva_cog else shrey_cog)
                    label_cog = label_cog.strip()
                    if (label_cog == ''):
                        continue
                    clean_sentence = clean(
                        row[0], stem=False, lemmatize=False, return_as_list=False)
                    X_data_temp.append(clean_sentence)
                    Y_data_temp.append(mapping_cog[label_cog])

        for t in threshold:
            X_data_temp_2 = get_filtered_questions(
                X_data_temp, threshold=t, what_type='os')
            X_data.extend(X_data_temp_2)
            X_data_orig.extend(X_data_temp)
            Y_data.extend(Y_data_temp)
    ...
    if shuffle and len(what_type) > 0:
        X_data, X_data_orig, Y_data = shuffle_generic(
            X_data, X_data_orig, Y_data)

    if include_keywords:
        domain_keywords = pickle.load(
            open(os.path.join(os.path.dirname(__file__), 'resources/domain.pkl'), 'rb'))
        for key in domain_keywords:
            for word in domain_keywords[key]:
                X_data.append(
                    clean(word, stem=False, lemmatize=False, return_as_list=True))
                X_data_orig.append(
                    clean(word, stem=False, lemmatize=False, return_as_list=False))
            ...
            for question in re.split('[\n][\s]*[\d]+\.', exercise_content[e].strip(), flags=re.M | re.DOTALL):
                if len(question) > 0:
                    X_data.append(re.sub('\n', ' ', re.sub(
                        '1\.', '', question.strip()), flags=re.M | re.DOTALL))
                    Y_data.append(e)
...
if __name__ == '__main__':
    ...
    data_os = {len(data_ada) + k[0]: k[1]
               for k in enumerate(list(zip(X_os, Y_os)))}
    data_bcl = {len(data_ada) + len(data_os) +
                k[0]: k[1] for k in enumerate(list(zip(X_bcl, Y_bcl)))}
