...
cog2label = {'Remember': 0, 'Understand': 1, 'Apply': 2,
             'Analyse': 3, 'Evaluate': 4, 'Create': 5}
label2cog = {v: k for k, v in cog2label.items()}

know2label = {'Metacognitive': 3, 'Procedural': 2,
              'Conceptual': 1, 'Factual': 0}
...
def create_paper(subject):
    ...
    for question in range(NO_OF_QUESTIONS):
        ...
        for student in range(NO_OF_STUDENTS):
            difficulty = (
                difficulty_know[know_label] + difficulty_cog[cog_label]) / 2
            r = random.random()
