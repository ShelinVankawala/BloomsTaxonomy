...
text = get_output(
    'pdftotext -table -fixed 0 -clip ../resources/%s -' % (pdfname + '.pdf'))
print('Parsing Done.')

toC = get_output('mutool show ../resources/%s outline' % (pdfname + '.pdf'))

if pdfname == 'DBMS':
    search_str = r'PART[\s]+1[\s]+Relational Databases'
# elif pdfname == 'OS2':
...
for line in list(map(str.strip, toC.split('\n'))):
    if re.search('Chapter', line) or '.' not in line:
        if pdfname == 'DBMS' or pdfname == 'OS3':
        ...
        if match:
            ...
            chapter_tree.append({'sno': match.group(1).strip(
            ), 'level': 1, 'title': title, 'pno': int(match.group(3).strip())})
    else:
        ...
        if match:
            title = re.sub(
                r'[^a-zA-Z0-9\-,/.?!\\\s:;\'"&$#@()\[\]{}]', '',  match.group(2).strip())
            if pdfname == 'DBMS':
                ...
                title = title.replace(
                    'Petersons Solution', 'Peterson\'s Solution')

            chapter_tree.append({'sno': match.group(1).strip(
            ), 'level': 2, 'title': title, 'pno': int(match.group(3).strip())})

# to ensure the last section is processed as per my current logic
chapter_tree += [{'sno': '[\d]*', 'level': -1,
                  'title': 'Exercises', 'pno': chapter_tree[-1]['pno'] + 1}]
...
pages = ('\n'.join(line for line in text.decode(
    'latin-1').split('\n') if line != '')).split('\f')
pages = ['\n'.join(['\n'] + p.split('\n')[:]) for p in pages]
...
for i, (cur_topic, next_topic) in enumerate(zip(chapter_tree[:-1], chapter_tree[1:])):
    ...
    if split_mode == SECTION_MODE:
        ...
        if pdfname == 'DBMS' or pdfname == 'OS2':
            t1 = t1.upper()
            t2 = t2.upper()
        ...
        try:
            section_text = re.split(t1.replace(
        ...
        section_text = re.split('\n[\s]*([\d]+\.[\d]+[\s]+)?' + t2.replace(
            ' ', '[\s]+'),  section_text, flags=re.M | re.DOTALL)[0]
...
with open('__Sections.csv', 'w') as csvfile:
    ...
    writer.writerows([[chapter_tree[i]['sno'], chapter_tree[i]['level'], chapter_tree[i]
                     ['title'], chapter_tree[i]['pno']] for i in range(len(chapter_tree[:-1]))])
