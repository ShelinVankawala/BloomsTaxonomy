...
__skip_files = {'__', '.DS_Store', 'Key Terms, Review Questions, and Problems',
                'Recommended Reading and Web Sites', 'Recommended Reading', 'Exercises', }
...
def __get_cleaned_section_text(subject, mode):
    ...
    for filename in sorted(os.listdir(os.path.join(os.path.dirname(__file__), path))):
        with open(path + filename, encoding='latin-1') as f:
            ...
            file_contents = clean_no_stopwords(
                file_contents, stem=False, lemmatize=True, as_list=False)
