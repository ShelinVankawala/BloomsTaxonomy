...
class Visualize:
    ...
    def get_probabilities(self, root, question):
    ...
    def populate_table(self, root, array_know, array_cog):
        nmarray = np.dot(np.array(array_know).reshape(-1, 1),
                         np.array(array_cog).reshape(1, -1))
        print(nmarray)
        frame = Frame(root)
        ...
        cog_values = ['', '', 'Remember', 'Understand',
                      'Apply', 'Analyze', 'Evaluate', 'Create']
        know_values = ['', '', 'Factual Knowledge', 'Conceptual Knowledge',
                       'Procedural Knowledge', 'Metacognitive Knowledge']

        # set your favourite rgb color
        myWhite = '#%02x%02x%02x' % (255, 255, 255)
        myTitleColor = '#%02x%02x%02x' % (
            255, 230, 190)  # set your favourite rgb color
        ...
        for x in range(1, 6):
            for y in range(1, 8):
                if x == 1 or y == 1:
                    if x == 1 and y == 1:
                        l = Label(
                            frame, text=know_values[x], relief=RIDGE, bg=myTitleColor)
                    elif y == 1:
                        l = Label(
                            frame, text=know_values[x], relief=RIDGE, bg=myTitleColor)
                    else:
                        l = Label(
                            frame, text=cog_values[y], relief=RIDGE, bg=myTitleColor)
                else:
                    var = nmarray[x-2, y-2] * 255
                    if (var != 0):
                        myCellColor = '#%02x%02x%02x' % (
                            255 - int(var)//2, 0, 0)  # set your heatmap color
                        l = Label(frame, text='{:.2f}'.format(
                            nmarray[x - 2][y - 2]), relief=RIDGE, bg=myCellColor)

                    else:
                        myCellColor = '#%02x%02x%02x' % (
                            255, 255, 255)  # set it white
        ...
        button = Button(frame, text='Refresh', command=lambda: self.create_table(
            root)).grid(row=6, column=4, pady=20, sticky=N+S+E+W)
        knowledge_label = Label(frame, text='Knowledge Dimension\n[[{:.2f} {:.2f}]\n[{:.2f} {:.2f}]]'.format(
            array_know[0], array_know[1], array_know[2], array_know[3])).grid(row=3, column=0, rowspan=3, sticky=N+S+E+W)
        cognitive_label = Label(frame, text='Cognitive Dimension\n[' + str(
            array_cog) + ']').grid(row=0, column=4, columnspan=2, sticky=N+S+E+W)
    ...
    def create_table(self, root):
        ...
        v = StringVar()
        question_label = Label(frame, text='Enter Question').grid(
            row=6, column=2, pady=10, sticky=N+S+E+W)

        e = Entry(frame, textvariable=v).grid(
            row=6, column=3, columnspan=3, pady=10, sticky=N+S+E+W)
        button = Button(frame, text='Submit', command=lambda: self.get_probabilities(
            root, v), relief=RIDGE).grid(row=7, column=4, pady=20, sticky=N+S+E+W)
        knowledge_label = Label(frame, text='Knowledge Dimension').grid(
            row=3, column=0, sticky=N+S+E+W)
        cognitive_label = Label(frame, text='Cognitive Dimension').grid(
            row=0, column=4, sticky=N+S+E+W)
