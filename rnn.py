...
class SkillClassifier:
    def __init__(self, input_dim=300, hidden_dim=128, dropout=0.2):
        ...
        # self.model.add(Embedding(input_dim, embedding_dim, input_length=max_sequence_length, weights=[
        #    embedding_matrix], trainable=False))
        self.model.add(
            SimpleRNN(hidden_dim, return_sequences=False, recurrent_dropout=dropout))
        self.model.add(
...
if __name__ == "__main__":
    ...
    joblib.dump(clf, os.path.join(
        os.path.dirname(__file__), 'models/BiRNN/rnn_model.pkl'))
